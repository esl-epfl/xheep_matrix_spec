...
Type *Type::getPrimitiveType(LLVMContext &C, TypeID IDNumber) {
  ...
  case RISCV_THEADMRTyID   : return getRISCV_THEADMRTy(C);
...
Type *Type::getRISCV_THEADMRTy(LLVMContext &C) { return &C.pImpl->RISCV_THEADMRTy; }
...
bool ArrayType::isValidElementType(Type *ElemTy) {
  return !ElemTy->isVoidTy() && !ElemTy->isLabelTy() &&
         !ElemTy->isMetadataTy() && !ElemTy->isFunctionTy() &&
         !ElemTy->isTokenTy() && !ElemTy->isX86_AMXTy() && !ElemTy->isRISCV_THEADMRTy();
}
