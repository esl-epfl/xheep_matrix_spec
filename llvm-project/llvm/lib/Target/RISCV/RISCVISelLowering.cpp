...
static MachineBasicBlock *emitTheadMatrixInsn(MachineInstr &MI,
                                                 MachineBasicBlock *BB,
                                                 const RISCVSubtarget &Subtarget) {
  auto ImmToTheadReg = [](unsigned Imm) {
  ...
  unsigned Opc;
  switch (MI.getOpcode()) {
    case RISCV::FMMACC_S_P:  {
      BuildMI(*BB, MI, MIMD, TII->get(RISCV::FMMACC_S),
        ImmToTheadReg(MI.getOperand(0).getImm()))
        .addReg(ImmToTheadReg(MI.getOperand(1).getImm()))
        .addReg(ImmToTheadReg(MI.getOperand(2).getImm()));    
      break;
    }
    case RISCV::MLD_W_P:  {
      BuildMI(*BB, MI, MIMD, TII->get(RISCV::MLD_W),
        ImmToTheadReg(MI.getOperand(0).getImm()))
    ...
    case RISCV::MST_W_P:  {
      BuildMI(*BB, MI, MIMD, TII->get(RISCV::MST_W))
        .addReg(ImmToTheadReg(MI.getOperand(0).getImm()))
        .addReg(MI.getOperand(1).getReg())
        .addReg(MI.getOperand(2).getReg());    
      break;
    }
...
RISCVTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case RISCV::MLD_W_P:
  case RISCV::MST_W_P:
    return emitTheadMatrixInsn(MI, BB, Subtarget);
