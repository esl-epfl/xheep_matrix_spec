...
#undef CASE_VMXOR_VMANDN_VMOR_OPCODES
  ...
  case ISD::INTRINSIC_VOID: {
    ...
    case Intrinsic::riscv_sf_vc_i_se:
      ...
      return;
    case Intrinsic::riscv_mld_w: {
      unsigned Opc = RISCV::MLD_W_P;
      SDLoc DL(Node);
      
      MachineSDNode *CNode;
      SDValue Ops[] = { Node->getOperand(2), Node->getOperand(3), Node->getOperand(4) };
      CNode = CurDAG->getMachineNode(Opc, DL, Node->getValueType(0), Ops);
